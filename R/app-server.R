...
app_server <- function(input, output, session) {
  ...
  check_pred <- reactive({
    data <- clean_data()
    if (
      "Concentration" %in% names(data) && length(data[["Concentration"]]) == 0L
    ) {
      return(tr("ui_hintdata", trans()))
    }
  ...
  plot_dist <- reactive({
    ...
    plot_distributions(
      dist,
      ylab = tr("ui_2ploty", trans()),
      lang = translation.value$lang
    )
  })
  ...
  plot_model_average <- reactive({
    ...
    percent <- if (
      !input$checkHc ||
        is.null(thresh_rv$percent) ||
        input$thresh_type == "Concentration"
    ) {
      NULL
    } else {
    ...
    validate(need(
      is.null(shape_data) | is.character(shape_data) | is.factor(shape_data),
    ...
    plot_predictions(
      ...
      shape = shape,
      percent = percent,
      label_adjust = input$adjustLabel,
      xaxis = input$xaxis,
      yaxis = input$yaxis,
      title = input$title,
      xmax = input$xMax,
  ...
  output$fitFail <- renderText({
    ...
    HTML(paste0(
      "<font color='grey'>",
      paste(fit_fail(), tr("ui_hintfail", trans())),
      "</font>"
    ))
  })
  ...
  shinyjs::onclick(
    "linkFormatFit",
    shinyjs::toggle("divFormatFit", anim = TRUE, animType = "slide", time = 0.2)
  )
  ...
  output$uiXmax <- renderUI({
    ...
    numericInput(
      ...
      label = "X-axis max",
      min = 1,
      value = max(data[[conc]], na.rm = TRUE)
    )
  })
  ...
  output$uiXmin <- renderUI({
    ...
    numericInput(
      ...
      label = "X-axis min",
      min = 1,
      value = min(data[[conc]], na.rm = TRUE)
    )
  })
  ...
  output$codeHead <- renderUI({
    if (
      upload.values$upload_state == "hot" && is.na(read_data()$Concentration[1])
    ) {
      return()
    }
  ...
  output$codeData <- renderUI({
    ...
    hot <- paste0(
      "data <- ",
      utils::capture.output(dput(clean_data())) %>% glue::glue_collapse()
    )
  ...
  output$codePredPlot <- renderUI({
    ...
    legend.colour <- ifelse(
      is.null(input$legendColour),
      "NULL",
      paste0("'", input$legendColour, "'")
    )
    legend.shape <- ifelse(
      is.null(input$legendShape) || input$legendShape == "-none-",
      "NULL",
      paste0("'", input$legendShape, "'")
    )
  ...
  output$codePredCl <- renderUI({
    ...
    c2 <- paste(
      "# use the nboot argument in",
      form,
      "to set the number of bootstrap samples"
    )
    conf <- paste0(
      paste0(form, "(dist, ", arg, " = "),
      thresh,
      "L, ci = TRUE",
      ", nboot = ",
      input$bootSamp %>% gsub(",", "", .) %>% as.integer(),
      "L)"
    )
    conf2 <- paste0(
      paste0(form, "(dist, ", arg, " = "),
      thresh,
      "L, ci = TRUE, average = FALSE",
      ", nboot = ",
      input$bootSamp %>% gsub(",", "", .) %>% as.integer(),
      "L)"
    )
  ...
  output$ui_1csvupload <- renderUI({
    fileInput(
      ...
      label = "",
      placeholder = tr("ui_1csvlabel", trans()),
      accept = c(".csv")
    )
  })
  ...
  output$ui_2rescale <- renderUI({
    checkboxInput("rescale", label = tr("ui_2rescale", trans()), value = TRUE)
  })
  ...
  output$ui_3ylab <- renderUI({
    textInput(
      "yaxis",
      value = "Percent of Species Affected",
  ...
  output$ui_userguide <- renderUI({
    if (translation.value$lang == "English") {
      return(includeMarkdown(system.file(
        package = "shinyssdtools",
        "extdata/user-en.md"
      )))
    }
